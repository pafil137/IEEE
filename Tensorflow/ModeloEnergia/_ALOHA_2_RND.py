...
def functionPowerR(_n =_n, _dist=_dist, _R=_R, _Nb=_Nb, _a=_a):
    ...
    for a in _a:        
        for dist in _dist:
            ...
            #Numero de Hops
            for n in _n:
                h = ((dist/3)*(math.pow((math.sqrt(2)+math.log(1+math.sqrt(2))),2)))/(math.sqrt((pi*math.pow(dist,2)/(2*n*teta)))*(2/teta)*(math.sin(teta/2)))
...
def functionPowerNb(_n =_n, _dist=_dist, _R=_R2, _Nb=_Nb2, _a=_a):
